...
embedding). `recall_facts` scans every fact item, then ranks them against the query with a plain
cosine-similarity scan in memory — no vector database involved. `MemoryTable` is provisioned at 25 read/write
capacity units, the ceiling of the AWS free tier, which comfortably covers this project's memory scale at no
cost. This was chosen over the alternatives for cost and operational simplicity:
...
`remember_fact` call is an independent `PutItem`, not a read-modify-write of one file, so concurrent
...
large. `MemoryTable` is created and wired up automatically by `template.yaml`; no manual setup is required
...
```bash
openapi-client$ sam build --use-container
```

The SAM CLI installs dependencies defined in each function's `requirements.txt` (`app/requirements.txt`, `agent/requirements.txt`), creates a deployment package, and saves it in the `.aws-sam/build` folder.
...
```bash
openapi-client$ sam local invoke MainFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
openapi-client$ sam local start-api
openapi-client$ curl http://localhost:3000/
```
...
```bash
openapi-client$ sam logs -n MainFunction --stack-name "openapi-client" --tail
```
...
```bash
openapi-client$ pip install -r tests/requirements.txt --user
# unit test
openapi-client$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
openapi-client$ AWS_SAM_STACK_NAME="openapi-client" python -m pytest tests/integration -v
```
...
# python.aws-lambda.openapi-client
