...
- `AgentFunction` — `AWS::Serverless::Function`, code at `agent/app.py` (handler `app.lambda_handler`) — `POST /agent`; an OpenAI tool-calling agent, reads `OPENAI_API_KEY` from SSM Parameter Store (`/openai-client/openai-api-key`).
