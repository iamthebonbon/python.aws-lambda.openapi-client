...
- `AgentFunction` — `AWS::Serverless::Function`, code at `agent/app.py` (handler `app.lambda_handler`) — `POST /agent`; an OpenAI tool-calling agent, reads `OPENAI_API_KEY` from SSM Parameter Store (`/openai-client/openai-api-key`). Its `remember_fact`/`recall_facts` tools give it long-term semantic memory: each fact is written directly to the `MemoryBucket` S3 bucket as its own object (text + embedding); `recall_facts` loads all fact objects into memory and ranks them against the query with a plain cosine-similarity scan. S3 is the memory store itself, not a snapshot of a local database, so no EFS mount or always-on database is needed and concurrent writers across containers are safe.
