...
- `AgentFunction` — `AWS::Serverless::Function`, code at `agent/app.py` (handler `app.lambda_handler`) — `POST /agent` and `POST /agent/{conversationId}`; an OpenAI tool-calling agent, reads `OPENAI_API_KEY` from SSM Parameter Store (`/openai-client/openai-api-key`). Its `remember_fact`/`recall_facts` tools give it long-term semantic memory: each fact is written directly to the `MemoryTable` DynamoDB table as its own item (text + embedding, keyed by `fact_id`); `recall_facts` scans all fact items into memory and ranks them against the query with a plain cosine-similarity scan. DynamoDB is the memory store itself, not a snapshot of a local database, so no EFS mount or always-on database is needed and concurrent writers across containers are safe. Provisioned throughput is capped at 25 RCU / 25 WCU to fit the AWS free tier. Conversation history is persisted the same way in a separate `ConversationTable`: one item per conversation, keyed by `conversation_id`, holding the full message history as a JSON blob. `POST /agent` (no path id) starts a new conversation and returns its `conversation_id` in the response; `POST /agent/{conversationId}` loads that conversation's history from DynamoDB before continuing it (404 if the id doesn't exist), and every request overwrites that conversation's single item with the updated history.
